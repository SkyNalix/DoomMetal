...
let drawRay windows_info level ray = 
    ...
    let r = if r = 0 then 0 else max 10 r - int_of_float (30. *. distance) in
    let g = if g = 0 then 0 else max 10 g - int_of_float (30. *. distance) in
    let b = if b = 0 then 0 else max 10 b - int_of_float (30. *. distance) in
